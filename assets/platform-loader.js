(() => {
  ...
  const ANALYTICS_KEY = 'solqvia-analytics-consent-v3';
  const valid = {
  ...
  const getAnalyticsConsent = () => { try { return JSON.parse(localStorage.getItem(ANALYTICS_KEY) || 'null'); } catch (_) { return null; } };
  const saveAnalyticsConsent = allowed => {
    const value = { allowed: !!allowed, updatedAt: new Date().toISOString(), policyVersion: cfg.analyticsConsentVersion || '' };
    try { localStorage.setItem(ANALYTICS_KEY, JSON.stringify(value)); } catch (_) {}
    return value;
  };
  ...
  const renderAnalyticsBanner = () => {
    if (getAnalyticsConsent()) return;
    const ja = document.documentElement.lang === 'ja';
    const bar = document.createElement('section'); bar.className = 'consent-bar'; bar.setAttribute('aria-label', ja ? 'アクセス解析の設定' : 'Analytics preferences');
    bar.innerHTML = `<div><strong>${ja ? 'アクセス解析について' : 'Analytics preferences'}</strong><p>${ja ? 'サイト改善のため、許可された場合のみアクセス解析を有効にします。広告の同意はGoogle認定CMPで別に管理します。' : 'Analytics loads only when allowed. Advertising consent is managed separately by a Google-certified CMP.'}</p></div><div class="consent-actions"><button type="button" data-consent="reject">${ja ? '拒否' : 'Reject'}</button><button type="button" class="button" data-consent="accept">${ja ? '解析を許可' : 'Allow analytics'}</button></div>`;
    document.body.appendChild(bar);
    bar.addEventListener('click', async e => { const b = e.target.closest('[data-consent]'); if (!b) return; const ok = b.dataset.consent === 'accept'; saveAnalyticsConsent(ok); bar.remove(); if (ok) { await enableAnalytics(); attachEvents(); } });
  };
  document.addEventListener('DOMContentLoaded', async () => {
    const consent = getAnalyticsConsent();
    if (consent?.allowed) { await enableAnalytics(); attachEvents(); }
