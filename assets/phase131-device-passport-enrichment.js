(()=>{
  ...
  let priority={priority_records:[]},realdb={records:[]},coverage={metrics:[]},taxonomy={domains:[]};
  ...
  function augment(){
    ...
    try{
      ...
      const parts=[];
      if(ex||real.length){
        parts.push(`<section class="p110-section p131-layer" data-p131-added><h3>${esc(t('Phase131 確認レベル','Phase131 evidence levels'))}</h3><div class="p131-evidence-grid"><div class="${real.length?'is-ok':''}"><b>${esc(t('SolQvia実機確認','SolQvia real-device'))}</b><span>${esc(real.length?t('あり（確認範囲のみ）','Available for tested scope'):t('なし','None'))}</span></div><div class="${ex?'is-ok':''}"><b>${esc(t('公式確認','Official sources'))}</b><span>${esc(ex?t('型番別詳細あり','Exact-model detail available'):t('既存DBを参照','Use base dataset'))}</span></div><div><b>${esc(t('外部動作確認','External field tests'))}</b><span>${esc(t('SolQvia実機と分離','Kept separate'))}</span></div><div><b>${esc(t('未確認','Unverified'))}</b><span>${esc(t('推測で埋めない','Never filled by inference'))}</span></div></div></section>`);
      }
      if(ex){
        parts.push(`<section class="p110-section p131-layer" data-p131-added><h3>${esc(t('Phase131 公式詳細データ','Phase131 official detail'))}</h3><div class="p110-kv">${objGrid(ex.identity)}${objGrid(ex.sim_esim)}${objGrid(ex.network)}${objGrid(ex.software)}</div><p class="p110-state">${esc(t('公式仕様は実際の通信成功を意味しません。SIM認識・データ・通話・SMS・5G・VoLTE/IMSは別々に確認します。','Official capability does not prove runtime success. SIM recognition, data, voice, SMS, 5G and VoLTE/IMS are evaluated separately.'))}</p><h4>${esc(t('実動作マトリクス','Runtime matrix'))}</h4>${runtime(ex)}<details class="p131-source-details"><summary>${esc(t('Phase131根拠URL','Phase131 evidence URLs'))}</summary><ul>${(ex.sources||[]).map(u=>`<li><a href="${esc(u)}" target="_blank" rel="noopener">${esc(u)}</a></li>`).join('')}</ul></details></section>`);
      }
      if(real.length){
  ...
  Promise.all([
    ...
    load('/assets/data/phase130-coverage-gap-audit-v1.json').catch(()=>({metrics:[]})),
    load('/assets/data/phase130-unified-capability-taxonomy-v1.json').catch(()=>({domains:[]}))
  ]).then(([p,r,c,tax])=>{priority=p;realdb=r;coverage=c;taxonomy=tax;foundation();augment();new MutationObserver(()=>augment()).observe(detail,{childList:true,subtree:false})});
})();