...
for p in htmls:
    ...
    if is_index:
        ...
        if soup.body and soup.body.get('data-release')!='phase32': errors.append(f'Missing release body marker: {p.relative_to(root)}')
...
for required in ['robots.txt','release-manifest.json','phase32-status.json','site.webmanifest','.well-known/security.txt']:
...
print(json.dumps({'phase':'32','html_files':len(htmls),'indexable':len(indexable),'sitemap_urls':len(sitemap_urls),'errors':errors[:100],'error_count':len(errors),'pass':not errors},ensure_ascii=False,indent=2))
